...
관리자는 매월 말일 7일 전부터 다음 달 7일까지 매일 사내망 로컬에서 S2 기준을 최신화한 뒤 repo에 커밋/푸시합니다. 사용자가 오늘 작업해야 하는데 기준 최신화가 되어 있으면 그대로 실행하고, 최신화가 안 되어 있으면 관리자에게 요청하거나 앱 본문 `2. S2 기준`에서 수동 S2 파일 업로드를 사용합니다.
...
- S2 최신화는 관리자 로컬 CLI로 실행하며, 실행 이력은 로컬 DB와 repo 요약 파일에 남깁니다.
...
관리자가 사내망 로컬에서 CLI로 S2 기준을 갱신합니다. Cloud 앱에는 S2 로그인/최신화 버튼을 노출하지 않습니다.
정기 갱신 창은 매월 말일 7일 전부터 다음 달 7일까지입니다. 이 기간에는 매일 기준을 갱신하고 repo에 커밋/푸시합니다.
